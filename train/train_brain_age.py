...
"""
train_brain_age.py
This file loads MRI files and train these files using 3D CNN.
Graph of val Loss, val MAE, scatter points on predicted age and actual age will be output after the training is finished.
"""
...
import numpy as np
import pandas as pd
import skimage.transform  # 本例不做resize
...
# Single model data generator
def mri_data_generator(df, batch_size=2, do_resize=False, new_shape=(64, 64, 64), shuffle=True, infinite=True):
    index = 0
    n = len(df)
    while True:
        ...
        for _ in range(batch_size):
            ...
            vol = data[0, 0, ...].astype(np.float32)  # 得到 (D,H,W)
            if do_resize:
...
def main():
    ...
    tf.keras.models.save_model(model, 'model_earlystopping_p=3_2025_4', save_format='tf')
    print("Model saved.")
    ...
    idx_A, idx_B = 10, 30
    plt.annotate('A', xy=(true_ages[idx_A], predictions.flatten()[idx_A]),
                 xytext=(true_ages[idx_A], true_ages[idx_A] - 5),
                 arrowprops=dict(facecolor='black', shrink=0.05))
    plt.annotate('B', xy=(true_ages[idx_B], predictions.flatten()[idx_B]),
                 xytext=(true_ages[idx_B], true_ages[idx_B] + 5),
                 arrowprops=dict(facecolor='black', shrink=0.05))
